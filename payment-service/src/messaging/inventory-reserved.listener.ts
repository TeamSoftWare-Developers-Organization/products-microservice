...
export class InventoryReservedListener {
  ...
  async handleInventoryReserved(@Payload() data: { orderId: string; userId?: string; totalAmount?: number; gateway?: string }) {
    ...
    try {
      ...
      switch (Boolean(!userId || totalAmount === undefined)) {
        case true: {
      ...
      console.error(`❌ [Payment Service] Error processing payment for Order ${orderId}:`, errMsg);
      this.paymentService.emitEvent('payment_failed', { orderId, reason: errMsg });
      this.paymentService.emitEvent('payment.failed', { orderId, reason: errMsg });
    }
  }
}
