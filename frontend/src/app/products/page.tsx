  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 p-8 bg-background min-h-screen">
      {products?.map((product) => (
        <div key={product.id} className="surface-card p-4 flex flex-col justify-between">
          <div>
            <img 
              src={resolveProductImage(product.imageUrl || product.main_image_url) || "/file.svg"} 
              alt={product.name || product.name_ar || "منتج"} 
              className="h-48 w-full object-cover rounded-lg" 
            />
            <h2 className="font-bold text-lg mt-3 text-foreground">{product.name || product.name_ar || "منتج"}</h2>
          ...
          <button
            onClick={async () => {
              try {
                await addToCart({ productId: String(product.id), quantity: 1 }).unwrap();
            ...
            disabled={isAdding}
            className="mt-4 w-full bg-emerald-600 hover:bg-emerald-500 disabled:bg-muted disabled:text-muted-foreground py-2.5 rounded-lg flex justify-center items-center gap-2 text-white font-medium transition-all"
          >
